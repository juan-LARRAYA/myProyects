...
const char* ssid = ING_WIFI_SSID;        // Cambia por el nombre de tu red WiFi
const char* password = ING_WIFI_PASSWORD;   // Cambia por la contraseña de tu red WiFi
...
const char* mqtt_server = "192.192.1.66"; // Dirección IP o dominio de tu broker EMQX
...
const char* mqtt_topic = "/INGENIA/ARG/automatizacion/ESP32/temperatura"; // Tema donde publicar
...
*/
  ...
  float temperature = 15.0 + random(0, 1000) / 100.0;
  char tempStr[8];
  dtostrf(temperature, 6, 2, tempStr); // Convierte la temperatura a string
  client.publish(mqtt_topic, tempStr);
